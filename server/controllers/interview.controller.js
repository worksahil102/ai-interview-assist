...
export const generateQuestion = async (req, res) => {
  try {
    ...
    if (!role || !mode || !experience) {
      return res
        .status(400)
        .json({ message: "Role , Expereince and Mode are required" });
    }
...
export const submitAnswer = async (req, res) => {
  try {
    ...
    if (timeTaken > question.timeLimit) {
      question.score = 0;
      question.feedback = " Your limit exceede . Answer not evaluated .";
...
export const finishInterview = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: `failed to finished interview ${error}` });
  }
};
